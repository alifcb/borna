    ...
    for (i = 0, len = mediaFiles.length; i < len; i += 1) {
        path = mediaFiles[i].fullPath;
		 document.getElementById('videov').value=path;
          document.getElementById('vidload').innerHTML='ثبت فیلم با موفقیت انجام شد';
    }
};
...
{
ttt=document.getElementById(id1).value;
ide=ttt.replace( /,/g, '' );
//alert(ide);
	document.getElementById(id2).value = FormatNumberBy3(ide);
}
function FormatNumberBy3(str) {
